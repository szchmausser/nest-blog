// Autenticacion 5.1
// Ahora que tenemos la Estrategia (`JwtStrategy`) y el Módulo (`AuthenticationModule`), podemos crear el Guard que los usará.
// Extiende de `AuthGuard('jwt')`. Gracias al paso 3 y 4, el string `'jwt'` ahora tiene significado.
// 1. Usa `Reflector` para leer `@Public()`.
// 2. Si es público → `return true`.
// 3. Si no → `super.canActivate(context)` (Ejecuta la JwtStrategy).
import {
  ExecutionContext,
  Injectable,
...
import { PUBLIC_ENDPOINT_DECORATOR } from '../decorators/public.decorator';
